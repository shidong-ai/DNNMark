...
def main():
  ...
  for row in reader:
    ...
    kepler_metrics_list.append(float(row[1]))
    pascal_metrics_list.append(float(row[2]))
  normalized = True
  if normalized == True:
    normalized_kepler_metrics_list = [a/b for a, b in zip(kepler_metrics_list,kepler_metrics_list)]
    normalized_pascal_metrics_list = [a/b for a, b in zip(kepler_metrics_list,pascal_metrics_list)]
  else:
    normalized_kepler_metrics_list = kepler_metrics_list
    normalized_pascal_metrics_list = pascal_metrics_list
  metrics_list = []
  metrics_list.append(normalized_kepler_metrics_list)
  metrics_list.append(normalized_pascal_metrics_list)
